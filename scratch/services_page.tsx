...
export default function ServicesPage() {
  ...
  // When selectedBranch changes, fetch services
  useEffect(() => {
    if (selectedBranch) {
      fetchServices();
    }
  ...
  const fetchServices = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/services?branch_id=${selectedBranch}`);
      const data = await res.json();
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/services/${id}?branch_id=${selectedBranch}`, { method: "DELETE" });
  ...
  const handleReactivate = async (id: string) => {
    if (!selectedBranch) return;
    if (!confirm("Are you sure you want to reactivate this service?")) return;
    
    try {
      const res = await fetch(`/api/admin/services/${id}/reactivate`, { 
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <main className="flex-1 p-8 lg:p-12 overflow-auto">
      <div className="flex justify-between items-end mb-8">
        <div>
          ...
          <p className="text-text-secondary mt-1 text-sm uppercase tracking-wider">Manage services and pricing for each branch.</p>
        </div>
              ...
              {services.map(s => (
                <tr key={s.id} className="hover:bg-bg-panel-elevated transition-colors">
                  ...
                  <td className="p-4 text-right">
                    ...
                    {s.is_active ? (
                      <button onClick={() => handleDelete(s.id)} className="p-2 text-text-secondary hover:text-accent-oxblood transition-colors inline-block ml-2" title="Deactivate"><Trash2 size={14}/></button>
                    ) : (
                      <button onClick={() => handleReactivate(s.id)} className="p-2 text-text-secondary hover:text-[#4ade80] transition-colors inline-block ml-2" title="Reactivate"><RotateCcw size={14}/></button>
                    )}
                  </td>
                </tr>
              ))}
              {services.length === 0 && (
                <tr><td colSpan={5} className="p-8 text-center text-text-secondary uppercase tracking-widest text-sm">No services found for this branch.</td></tr>
              )}
            </tbody>
          </table>
        </div>
