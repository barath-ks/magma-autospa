  ...
  const handleUsernameApprove = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        fetchUsernameRequests();
        fetchUsers(); // Refresh the users table to show new ID
      } else {
        alert(data.error);
      }
    } catch (e) {
        ...
        {usernameRequests.length > 0 && (
          <div className="mb-10 panel p-6 border-l-[3px] border-l-accent-oxblood">
            <h2 className="text-sm font-semibold uppercase tracking-widest text-text-primary mb-4 flex items-center gap-2">
              <AlertCircle size={16} className="text-accent-oxblood" /> Pending Manager ID Changes
            </h2>
              ...
              {usernameRequests.map(req => (
                <div key={req.id} className="bg-bg-base border border-border-hairline p-4 flex flex-col md:flex-row justify-between items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="font-mono text-sm text-accent-oxblood">{req.requested_login_id}</div>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => handleUsernameApprove(req.id)} disabled={usernameActionLoading} className="px-3 py-1.5 bg-accent-oxblood text-white text-[10px] uppercase tracking-widest font-bold hover:bg-opacity-90 transition-colors disabled:opacity-50 flex items-center gap-1">
