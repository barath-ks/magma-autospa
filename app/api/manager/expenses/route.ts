...
export async function GET(request: Request) {
  ...
  try {
    let listSql = `
      ...
      WHERE e.created_at >= NOW() - INTERVAL '${intervalStr}'
    `;
    let sumSql = `
    ...
    if (branchId) {
      listSql += ` AND e.branch_id = $1`;
      sumSql += ` AND branch_id = $1`;
      args.push(branchId);
    }
    ...
    const sumRes = await db.query(sumSql, args);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const id = uuidv4();
    await db.query(
      `INSERT INTO expenses (id, description, amount, category, branch_id) VALUES ($1, $2, $3, $4, $5)`,
      [id, description, amount, category, branchId]
    );
